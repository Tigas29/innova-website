import styled from 'styled-components';

const CtaSection = styled.section`
  padding: 80px 28px;
  background: var(--grad);
`;

const CtaInner = styled.div`
  max-width: 620px; margin: 0 auto; text-align: center;
`;

const CtaH2 = styled.h2`
  font-family: var(--font-display);
  font-size: clamp(24px, 2.8vw, 38px);
  font-weight: 700; color: var(--branco);
  margin-bottom: 14px; line-height: 1.15;
`;

const CtaText = styled.p`
  font-size: 16px; color: rgba(255,255,255,0.8);
  line-height: 1.75; margin-bottom: 32px;
`;

const Btns = styled.div`
  display: flex; gap: 12px; justify-content: center; flex-wrap: wrap;
`;

const BtnWhite = styled.button`
  display: inline-flex; align-items: center;
  background: var(--branco); color: var(--quase-preto);
  font-family: var(--font-display); font-weight: 700; font-size: 15px;
  padding: 14px 28px; border-radius: 50px; border: none; cursor: pointer;
  &:hover { opacity: 0.92; transform: translateY(-2px); }
`;

const BtnOutlineA = styled.a`
  display: inline-flex; align-items: center;
  border: 2px solid rgba(255,255,255,0.55); color: var(--branco);
  font-family: var(--font-display); font-weight: 600; font-size: 15px;
  padding: 12px 26px; border-radius: 50px; text-decoration: none;
  &:hover { background: rgba(255,255,255,0.1); }
`;

const Trust = styled.div`
  display: flex; gap: 16px; justify-content: center;
  margin-top: 20px; flex-wrap: wrap;
  font-family: var(--font-display); font-size: 13px;
  color: rgba(255,255,255,0.6);
`;

const FooterMin = styled.footer`
  background: var(--quase-preto); padding: 20px;
  text-align: center; font-family: var(--font-display);
  font-size: 13px; color: rgba(255,255,255,0.35);
`;

const WaFloat = styled.a`
  position: fixed; bottom: 28px; right: 28px; z-index: 400;
  background: #25D366; color: var(--branco);
  width: 56px; height: 56px; border-radius: 50%;
  display: flex; align-items: center; justify-content: center;
  text-decoration: none; box-shadow: 0 8px 24px rgba(37,211,102,0.4);
  &:hover { transform: scale(1.08); }
  svg { width: 28px; height: 28px; }
`;

export default function CTAFinal({ onCtaClick, waUrl }) {
  return (
    <>
      <CtaSection>
        <CtaInner>
          <CtaH2>O melhor momento para começar era ontem. O segundo melhor é agora.</CtaH2>
          <CtaText>
            Uma avaliação de 40 minutos pode mudar a forma como você se relaciona com o seu corpo.
            Sem compromisso.
          </CtaText>
          <Btns>
            <BtnWhite onClick={onCtaClick}>Agendar Avaliação →</BtnWhite>
            <BtnOutlineA href={waUrl} target="_blank">Falar pelo WhatsApp</BtnOutlineA>
          </Btns>
          <Trust>
            <span>★ 5.0 no Google</span>
            <span>·</span>
            <span>Santo Agostinho, BH</span>
            <span>·</span>
            <span>Respondemos em até 2h</span>
          </Trust>
        </CtaInner>
      </CtaSection>

      <FooterMin>
        INNOVA MOVIMENTO · Rua Araguari, 1750, Sl 800 · Santo Agostinho · BH · CREFITO/MG: RE 007640 · © 2026
      </FooterMin>

      <WaFloat href={waUrl} target="_blank" aria-label="WhatsApp">
        <svg viewBox="0 0 24 24" fill="currentColor">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      </WaFloat>
    </>
  );
}
